...
## --- Self-defined structures --- (to be placed into own script)

def rect_trimer(step, L1,W1,L2,W2,L3,W3,G1,G2,H, mesh='cube', ORIENTATION=1):
    """
    Generate rectangular wire dimer with long axis along X on X/Y plane
    ...
    ## wire 1
    STRUCT0 = rect_wire(step, L=L1,H=H,W=W2, mesh=mesh, ORIENTATION=ORIENTATION).T
    ## wire 2
    STRUCT1 = rect_wire(step, L=L2,H=H,W=W2, mesh=mesh, ORIENTATION=ORIENTATION).T
    ## wire 3
    STRUCT2 = rect_wire(step, L=L3,H=H,W=W3, mesh=mesh, ORIENTATION=ORIENTATION).T
    
    ## shift and concatenate wires
    STRUCT0[0] -= (L2 + G1)*step
    STRUCT0 = STRUCT0.T
    
    STRUCT1 = STRUCT1.T
    
    STRUCT2[0] += (L2 + G2)*step
    STRUCT2 = STRUCT2.T

    dipoles = np.concatenate( [STRUCT0, STRUCT1, STRUCT2] )
    
    return dipoles

def color_metamolecule(step, L, H, W, mesh='cube', ORIENTATION=1, struct_state = 'all'):
    """
    Generate rectangular wire dimer with long axis along X on X/Y plane
    ...
    if struct_state == 'all':
        ## wire G
        #STRUCT_G = rect_wire(step, L=100.7338691/step,H=350/step,W=87.92479635/step, mesh=mesh, ORIENTATION=ORIENTATION).T
        STRUCT_G = rect_wire(step, L=L,H=H,W=W, mesh=mesh, ORIENTATION=ORIENTATION).T
        rotate = -45
        rotate *= np.pi/180.
        rotX = STRUCT_G[0]*np.cos(rotate) - STRUCT_G[1]*np.sin(rotate)
        rotY = STRUCT_G[0]*np.sin(rotate) + STRUCT_G[1]*np.cos(rotate)
        STRUCT_G[0]=rotX
        STRUCT_G[1]=rotY

        ## wire B

        # STRUCT_B1 = rect_wire(step, L=L,H=350/step,W=W, mesh=mesh, ORIENTATION=ORIENTATION).T
        STRUCT_B1 = rect_wire(step, L=87.4132798/step,H=350/step,W=42.1677106/step, mesh=mesh, ORIENTATION=ORIENTATION).T


        rotate = 45
        rotate *= np.pi/180.
        rotX = STRUCT_B1[0]*np.cos(rotate) - STRUCT_B1[1]*np.sin(rotate)
        rotY = STRUCT_B1[0]*np.sin(rotate) + STRUCT_B1[1]*np.cos(rotate)
        STRUCT_B1[0]=rotX
        STRUCT_B1[1]=rotY
        ## wire B

        # STRUCT_B2 = rect_wire(step, L=L,H=350/step,W=W, mesh=mesh, ORIENTATION=ORIENTATION).T
        STRUCT_B2 = rect_wire(step, L=87.4132798/step,H=350/step,W=42.1677106/step, mesh=mesh, ORIENTATION=ORIENTATION).T


        rotate = 45
        rotate *= np.pi/180.
        rotX = STRUCT_B2[0]*np.cos(rotate) - STRUCT_B2[1]*np.sin(rotate)
        rotY = STRUCT_B2[0]*np.sin(rotate) + STRUCT_B2[1]*np.cos(rotate)
        STRUCT_B2[0]=rotX
        STRUCT_B2[1]=rotY

        # wire R
        ##STRUCT_R = rect_wire(step, L=148/step,H=350/step,W=114/step, mesh=mesh, ORIENTATION=ORIENTATION).T
        # STRUCT_R = rect_wire(step, L=144.4283703/step,H=350/step,W=90.4916349/step, mesh=mesh, ORIENTATION=ORIENTATION).T
        # rotate = 90
        # rotate *= np.pi/180.
        # rotX = STRUCT_R[0]*np.cos(rotate) - STRUCT_R[1]*np.sin(rotate)
        # rotY = STRUCT_R[0]*np.sin(rotate) + STRUCT_R[1]*np.cos(rotate)
        # STRUCT_R[0]=rotX
        # STRUCT_R[1]=rotY
        ...
        STRUCT_B1 = STRUCT_B1.T
        STRUCT_B2 = STRUCT_B2.T
        STRUCT_G = STRUCT_G.T
        # STRUCT_R = STRUCT_R.T

        # dipoles = np.concatenate( [STRUCT_G, STRUCT_B1, STRUCT_B2, STRUCT_R] )
        # dipoles = np.concatenate( [STRUCT_G, STRUCT_R] )
        dipoles = np.concatenate( [STRUCT_G, STRUCT_B1, STRUCT_B2] )

    elif struct_state == 'blue':
        ## wire B
        STRUCT_B1 = rect_wire(step, L=L,H=H,W=W, mesh=mesh, ORIENTATION=ORIENTATION).T
        rotate = 45         # originally 45
        rotate *= np.pi/180.
        rotX = STRUCT_B1[0]*np.cos(rotate) - STRUCT_B1[1]*np.sin(rotate)
        rotY = STRUCT_B1[0]*np.sin(rotate) + STRUCT_B1[1]*np.cos(rotate)
        STRUCT_B1[0]=rotX
        ...
        ## wire B
        STRUCT_B2 = rect_wire(step, L=L,H=H,W=W, mesh=mesh, ORIENTATION=ORIENTATION).T
        rotX = STRUCT_B2[0]*np.cos(rotate) - STRUCT_B2[1]*np.sin(rotate)
        rotY = STRUCT_B2[0]*np.sin(rotate) + STRUCT_B2[1]*np.cos(rotate)
        STRUCT_B2[0]=rotX
        STRUCT_B2[1]=rotY
        ...
        dipoles = np.concatenate( [STRUCT_B1, STRUCT_B2] )

    elif struct_state == 'green':
        ## wire G
        STRUCT_G = rect_wire(step, L=H,H=H,W=W, mesh=mesh, ORIENTATION=ORIENTATION).T
        ...
        dipoles = np.concatenate( [STRUCT_G] )

    else:
        # wire R
        STRUCT_R = rect_wire(step, L=L,H=H,W=W, mesh=mesh, ORIENTATION=ORIENTATION).T
        ...
        dipoles = np.concatenate( [STRUCT_R] )

    return dipoles

def disc_rect_dimer(step, R, L,H,W,G, mesh1='hex', mesh2='cube', ORIENTATION=1):
    """
    Generate rectangular wire dimer with long axis along X on X/Y plane
    ...
    ## wire 1
    STRUCT0 = nanodisc(step, R=R,H=H, mesh=mesh1, ORIENTATION=ORIENTATION).T
    ## wire 2
    STRUCT1 = rect_wire(step, L=L,H=H,W=W, mesh=mesh2, ORIENTATION=ORIENTATION).T
    
    
    ## shift and concatenate wires
    STRUCT0[0] -= (R + G/2.)*step
    STRUCT0 = STRUCT0.T
...
def leaf_gielis(step, n1, n2, n3, sizing_ratio, H, mesh='cube', ORIENTATION=1):

    unit_cell_size = int(np.ceil(180/step))     # 200 nm (cell periodicity), using 20 nm as buffer
    #n_sides = 100
    #theta = np.linspace(0, 2*math.pi, n_sides+1)
    #theta = theta[1:n_sides]
    # r = 1 / (np.abs(np.cos(2*theta/4)/1)**n2 + (np.abs(np.sin(2*theta/4)/1))**n3)**(1/n1)

    def conditionLeaf(xi,yi,zi):

        theta = math.atan2(yi,xi)
        r = (1 / (np.abs(np.cos(2*theta/4)/1)**n2 + (np.abs(np.sin(2*theta/4)/1))**n3)**(1/n1)) * sizing_ratio
        InLeaf = ((abs(zi) < H) 
                    and 
                    (np.sqrt(xi**2+yi**2)<(r+0.2))
                )
        OutCell = (r > unit_cell_size/2 or r < -unit_cell_size/2)

        return InLeaf, OutCell
    
    if mesh == 'cube':
        sp = _meshCubic_edited( (-1*unit_cell_size/2, 1*unit_cell_size/2), (-1*unit_cell_size/2, 1*unit_cell_size/2), 
                         (0, int(np.ceil(H))),  conditionLeaf)
    elif mesh == 'hex':
        sp = _meshHexagonalCompact_edited(  (-1*unit_cell_size/2, 1*unit_cell_size/2), 
                                     (-1*unit_cell_size/2, 1*unit_cell_size/2), (0, int(np.ceil(H))),
                                    conditionLeaf, ORIENTATION)
...
#def color_leaf_gielis_metamolecule(step, n1, n2, n3, sizing_ratio, H, mesh='cube', ORIENTATION=1, struct_state = 'all'):
def color_leaf_gielis_metamolecule(step, n1, n2, sizing_ratio, H, mesh='cube', ORIENTATION=1, struct_state = 'all'):
    """
    Generate rectangular wire dimer with long axis along X on X/Y plane
    ...
    if struct_state == 'all':
        ...
        STRUCT_G = leaf_gielis(step, n1 = 71.97003309, n2 = 272.50815525, n3 = 272.50815525, sizing_ratio = 1.62059285, H = H, mesh=mesh, ORIENTATION=ORIENTATION).T
        rotate = 90        # originally -45
        rotate *= np.pi/180.
        rotX = STRUCT_G[0]*np.cos(rotate) - STRUCT_G[1]*np.sin(rotate)
        rotY = STRUCT_G[0]*np.sin(rotate) + STRUCT_G[1]*np.cos(rotate)
        STRUCT_G[0]=rotX
        ...
        # STRUCT_B1 = leaf_gielis(step, n1 = 0.5, n2 = 0.5, n3 = 0.5, sizing_ratio = 5.424215047, H = (23.33333377*15)/15, mesh=mesh, ORIENTATION=ORIENTATION).T


        # rotate = 0        # originally 45
        # rotate *= np.pi/180.
        # rotX = STRUCT_B1[0]*np.cos(rotate) - STRUCT_B1[1]*np.sin(rotate)
        # rotY = STRUCT_B1[0]*np.sin(rotate) + STRUCT_B1[1]*np.cos(rotate)
        # STRUCT_B1[0]=rotX
        ...
        # STRUCT_B2 = leaf_gielis(step, n1 = 0.5, n2 = 0.5, n3 = 0.5, sizing_ratio = 5.424215047, H = (23.33333377*15)/15, mesh=mesh, ORIENTATION=ORIENTATION).T

        # rotate = 0         # orginally 45
        # rotate *= np.pi/180.
        # rotX = STRUCT_B2[0]*np.cos(rotate) - STRUCT_B2[1]*np.sin(rotate)
        # rotY = STRUCT_B2[0]*np.sin(rotate) + STRUCT_B2[1]*np.cos(rotate)
        # STRUCT_B2[0]=rotX
        ...
        # wire R
        STRUCT_R = leaf_gielis(step, n1 = n1, n2 = n2, n3 = n2, sizing_ratio = sizing_ratio, H = H, mesh=mesh, ORIENTATION=ORIENTATION).T
        ...
        ## wire B
        STRUCT_B1 = leaf_gielis(step, n1 = n1, n2 = n2, n3 = n2, sizing_ratio = sizing_ratio, H = H, mesh=mesh, ORIENTATION=ORIENTATION).T
        rotate = -45         # originally 45
        rotate *= np.pi/180.
        rotX = STRUCT_B1[0]*np.cos(rotate) - STRUCT_B1[1]*np.sin(rotate)
        rotY = STRUCT_B1[0]*np.sin(rotate) + STRUCT_B1[1]*np.cos(rotate)
        ...
        ## wire B
        STRUCT_B2 = leaf_gielis(step, n1 = n1, n2 = n2, n3 = n2, sizing_ratio = sizing_ratio, H = H, mesh=mesh, ORIENTATION=ORIENTATION).T
        rotX = STRUCT_B2[0]*np.cos(rotate) - STRUCT_B2[1]*np.sin(rotate)
        rotY = STRUCT_B2[0]*np.sin(rotate) + STRUCT_B2[1]*np.cos(rotate)
        STRUCT_B2[0]=rotX
        STRUCT_B2[1]=rotY
        ...
        ## wire G
        STRUCT_G = leaf_gielis(step, n1 = n1, n2 = n2, n3 = n2, sizing_ratio = sizing_ratio, H = H, mesh=mesh, ORIENTATION=ORIENTATION).T
...
# if cubic grid falls outside of unit cell size, clear the structure (we want a full leaf-shaped structure, not a clipped-off leaf-shaped structure)
def _meshCubic_edited(XRANGE, YRANGE, ZRANGE, condition):
    """mesh on a cubic grid
    ...
    for xi in range(int(XRANGE[0]), int(XRANGE[1])):
        for yi in range(int(YRANGE[0]), int(YRANGE[1])):
            for zi in range(int(ZRANGE[0]), int(ZRANGE[1])):
                ...
                if condition(xi, yi, zi)[0]:
                    sp.append([xi,yi,zi])
                if condition(xi, yi, zi)[1]:
                    sp = [[0,0,0]]
                    return sp
...
# if cubic grid falls outside of unit cell size, clear the structure (we want a full leaf-shaped structure, not a clipped-off leaf-shaped structure)
def _meshHexagonalCompact_edited(XRANGE, YRANGE, ZRANGE, condition, ORIENTATION=1):
    """mesh on a hexagonal compact grid
...
STRUCT_LIST = [sphere, spheroid, rect_wire, nanorod, nanocone, 
               ...
               split_ring, rect_split_ring, rect_trimer, color_metamolecule, disc_rect_dimer]